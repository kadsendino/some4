...
# class Regime(Scene):
    ...
    def regime(self,useed_k):
        # # === Formel: useed^k hervorheben ===
        # number = MathTex(
        #     r"(-1)^{sign} \cdot useed^k \cdot 2^{exponent} \cdot (1+fraction)",
        #     substrings_to_isolate=[
        #         r"(-1)", r"^{sign}", r"useed", r"^k", r"2", r"^{exponent}", r"(1+", r"fraction", r")"
        #     ]
        # )
        #
        # number.set_color_by_tex("^{sign}", RED)
        # number.set_color_by_tex("^k", YELLOW)
        # number.set_color_by_tex("^{exponent}", BLUE)
        # number.set_color_by_tex("fraction", GREEN)
        #
        # self.play(Write(number))
        ...
        # # === Zoom in auf useed^k ===
        # useed_k = VGroup(
        #     *number.get_parts_by_tex("useed"),
        #     *number.get_parts_by_tex("^k")
        # )
        # others = VGroup(*[part for part in number if part not in useed_k])
        #
        # self.play(
        #     others.animate.set_opacity(0),
        #     useed_k.animate.set_opacity(1),
        #     run_time=0.5
        # )
        # self.play(
        #     useed_k.animate.scale(2).move_to(ORIGIN),
        #     run_time=1
        # )
        # self.wait()
